...
abstract class InvoicePrinterBase
{
  ...
  protected function printInfo()
  {
    ...
    elseif ($invoiceData['state_id'] == 6)
      ...
      $pdf->Cell(68, 5, $GLOBALS['locPDFInvoiceHeader'], 0, 1, 'R');
    $pdf->SetFont('Helvetica','',10);
    $pdf->SetXY($info_x, $pdf->GetY()+5);
    ...
    if ($recipientData['company_id'])
    {
      ...
      $pdf->Cell(28, 5, $recipientData['company_id'], 0, 1, 'R');
    }
    $pdf->SetX($info_x);
    $pdf->Cell(40, 5, $GLOBALS["locPDF${locStr}Number"] . ': ', 0, 0, 'R');
    $pdf->Cell(28, 5, $invoiceData['invoice_no'], 0, 1, 'R');
    $pdf->SetX($info_x);
    ...
    $pdf->Cell(28, 5, $strInvoiceDate, 0, 1, 'R');
    if ($this->printStyle == 'invoice')
    {
      $pdf->SetX($info_x);
      $pdf->Cell(40, 5, $GLOBALS['locPDFDueDate'] . ': ', 0, 0, 'R');
      $pdf->Cell(28, 5, $strDueDate, 0, 1, 'R');
      $pdf->SetX($info_x);
      ...
      $pdf->Cell(28, 5, sprintf(getSetting('invoice_terms_of_payment'), $paymentDays), 0, 1, 'R');
      $pdf->SetX($info_x);
      $pdf->Cell(40, 5, $GLOBALS['locPDFPeriodForComplaints'] . ': ', 0, 0, 'R');
      $pdf->Cell(28, 5, getSetting('invoice_period_for_complaints'), 0, 1, 'R');
      $pdf->SetX($info_x);
      $pdf->Cell(40, 5, $GLOBALS['locPDFPenaltyInterest'] . ': ', 0, 0, 'R');
      $pdf->Cell(28, 5, $this->_formatNumber(getSetting('invoice_penalty_interest'), 1, true) . ' %', 0, 1, 'R');
      $pdf->SetX($info_x);
      if ($this->refNumber)
      {
        $pdf->Cell(40, 5, $GLOBALS['locPDFInvoiceRefNr'] . ': ', 0, 0, 'R');
        $pdf->Cell(28, 5, $this->refNumber, 0, 1, 'R');
      }
    }
    ...
    if ($invoiceData['reference'] && $this->printStyle != 'dispatch')
    {
      ...
      $pdf->Cell(28, 5, $invoiceData['reference'], 0, 1, 'R');
    }
